...
class Helper:
    ...
    def get_title(self, href: str, m_i_detail: BeautifulSoup) -> str:
        try:
            heading_name = m_i_detail.find("h2", class_="heading-name").text.strip("\n")
            return heading_name
        except Exception as e:
            self.error_log(
    ...
    def get_description(self, href: str, m_i_detail: BeautifulSoup) -> str:
        try:
            description = (
                m_i_detail.find("div", class_="description").text.strip("\n").strip()
            )
            return description
        except Exception as e:
            self.error_log(
    ...
    def get_cover_url(self, href: str, m_i_detail: BeautifulSoup) -> str:
        try:
            m_i_d_poster = m_i_detail.find("div", class_="m_i-d-poster")
            film_poster_img = m_i_d_poster.find("img")
            img_src = film_poster_img.get("src")
            if not img_src.startswith("https://"):
                img_src = CONFIG.FMOVIESTO_HOMEPAGE + img_src
    ...
    def get_extra_info(self, m_i_detail: BeautifulSoup) -> dict:
        extra_info = {}
        try:
            stats = m_i_detail.find("div", class_="stats")
            stats_spans = stats.find_all("span")
            ...
            row_lines = m_i_detail.find_all("div", class_="row-line")
            for row_line in row_lines:
                row_line_text = row_line.text.strip("\n").strip()
                if ":" not in row_line_text:
                    row_line_text += ":"
                k, v = row_line_text.split(":")
                v = v.replace("\n", "")
                extra_info[k] = v
